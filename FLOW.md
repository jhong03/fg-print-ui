...
3. **Pick the job:**
   ...
   - **This tab is QR-gated:** the label does **not** print yet. A task-list appears
     (☐ **Green/Start** ☐ **Red/End**). **Scan this workcell's two engraved QR tags**
     — the tag's 2nd digit decides Green (`1`) vs Red (`2`); the scanned number shows
     under the pill. Only when both a Green and a Red are scanned does the Work Order
     label go to the print queue. Scanned the wrong colour? Just rescan the right one
     (it replaces). The **Print label** button stays disabled until both are in (no
     bypass).
...
above once a Green (Start) and a Red (End) tag are scanned. Each tag is a 4-digit
number where **only the 2nd digit gates direction** (`1` = Start/Green, `2` =
End/Red); the 1st digit (workcell) and last two (sequence) are NOT validated — any
4-digit tag whose 2nd digit is 1 or 2 binds, and rescanning a colour replaces it
(e.g. change Green from `2101` to `2102`). Anything that isn't such a tag is
rejected. On release the bind is
...
| `locations.json` | `requireQrBinding` | QR binding gate on this tab (`qrWorkcell` is now unused — gating reads only the tag's 2nd digit) |
...
and resolves the job. QR binding tags are 4-digit numbers whose **2nd digit** gates
direction (1=Start/Green, 2=End/Red); workcell + sequence digits are not validated.
